...
func (m *Module) RegisterPolicies(reg *authz.Registry) {
	...
	reg.MustRegister("GET", "/api/v1/aiops/memories/audit", authz.Policy{Resource: "aiops", Action: "view", Scope: authz.ScopePlatform})
	...
	reg.MustRegister("POST", "/api/v1/aiops/memories/batch-forget", authz.Policy{Resource: "aiops", Action: "delete", Scope: authz.ScopePlatform})
	reg.MustRegister("GET", "/api/v1/aiops/memory-metrics", authz.Policy{Resource: "aiops", Action: "view", Scope: authz.ScopePlatform})
	reg.MustRegister("GET", "/api/v1/aiops/memory-metrics/review-samples", authz.Policy{Resource: "aiops_config", Action: "edit", Scope: authz.ScopePlatform})
	reg.MustRegister("PUT", "/api/v1/aiops/memory-metrics/:id/review", authz.Policy{Resource: "aiops_config", Action: "edit", Scope: authz.ScopePlatform})
...
func (m *Module) RegisterRoutes(ctx *module.Context, protected *gin.RouterGroup) {
	...
	g := protected.Group("/aiops")
	{
		...
		g.GET("/memories/audit", m.handler.ListMemoryAudits)
		...
		g.POST("/memories/batch-forget", m.handler.BatchForgetMemories)
		g.GET("/memory-metrics", m.handler.GetMemoryMetrics)
		g.GET("/memory-metrics/review-samples", m.handler.ListMetricReviewSamples)
		g.PUT("/memory-metrics/:id/review", m.handler.ReviewMetricAssertion)
