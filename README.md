...
- **AI Agent**（原生 Tool Calling）
  ...
  - Agent 暂存写操作在生产集群默认双人审批；变更中心显示诊断证据摘要、预览差异、审批/观察/回滚状态，支持取消并导出审计 JSON。自动回滚目前只覆盖 Deployment 创建、扩缩容、更新；生产自动通道拒绝删除、任意 YAML、hostPath 挂载和环境变量值变更。平台其他手工写接口尚未统一纳入该审批链
...
以下清单用于开发/演示，包含单副本和本地依赖，不适合作为生产安装包。生产模板与数据库恢复验收见 [`deploy/k8s/PRODUCTION.md`](deploy/k8s/PRODUCTION.md)。
...
2. **保护 JWT 与加密密钥** - 使用独立强随机字符串；示例占位值会在启动时被拒绝，历史加密数据需要保留原加密密钥
